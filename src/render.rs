...
impl Compiler {
    pub fn new(minify: bool) -> Result<Self> {
        ...
        let css = format!("{}\n{highlight_css}", include_str!("../assets/slides.css"));
        let javascript = include_str!("../assets/slides.js");
        Ok(Self {
            dependencies: BTreeSet::new(),
            highlighter,
            css: if minify {
                crate::bundle::css(&css)?
            } else {
                css
            },
            javascript: if minify {
                crate::bundle::javascript(javascript)?
            } else {
                javascript.into()
            },
            minify,
                ...
                match value {
                    ...
                    NodeValue::Image(mut image) => {
                        if let Some((source, suffix)) = image_path(&image.url, parent)? {
                            ...
                            let target =
                                format!("assets/{}.{}", crate::build::digest(&bytes), extension);
